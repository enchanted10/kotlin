...
internal class KtUltraLightClass(kt: KtClassOrObject) : KtLightClassImpl(kt) {
    ...
    private fun propertyField(prop: KtProperty, usedNames: HashSet<String>, forceStatic: Boolean): KtLightField? {
        ...
        if ((prop.initializer == null || forceStatic && !prop.isVar) && prop.accessors.isNotEmpty()) return null
        ...
        if (prop.hasModifier(KtTokens.PRIVATE_KEYWORD))
            modifiers.add(PsiModifier.PRIVATE)
        else if (prop.hasModifier(KtTokens.CONST_KEYWORD) || prop.hasModifier(KtTokens.LATEINIT_KEYWORD))
